...
const UploadModal: React.FC<UploadModalProps> = ({ 
        ...
        try {
            ...
            setExtractedFabrics(prev => {
                ...
                if (type === 'main') {
                    ...
                    if (rawName) {
                        const newName = toSentenceCase(rawName);
  ...
  const analyzeFileGroup = async (groupFiles: File[], groupName: string): Promise<Partial<Fabric>> => {
      ...
      try {
        if (pdfFile) {
            ...
            if (base64Data.length < 1000000) rawData.pdfUrl = base64Data;
      ...
      if (!rawData.name || rawData.name === "Unknown") rawData.name = cleanFabricName(groupName); 
      ...
      else rawData.supplier = "CONSULTAR";
      ...
      for (const file of imgFiles) {
        processedCount++;
        if (processedCount % 3 === 0) setCurrentProgress(`Escaneando colores (${processedCount}/${imgFiles.length}) para ${rawData.name}...`);
        ...
        try {
            ...
            if (!detectedName) {
                ...
                if (!detectedName) {
                    ...
                    detectedName = cleanColorName.replace(/[-_]/g, " ").trim();
                }
            }
      ...
      if (Object.keys(colorImages).length > 0) {
          ...
          try { mainImageToUse = await compressImage(imgFiles[0], 2560, 0.95); } catch(e) { mainImageToUse = ''; }
      }
  ...
  const updateFabricField = (index: number, field: keyof Fabric, value: any) => {
      setExtractedFabrics(prev => {
          ...
          if (field === 'name') finalValue = toSentenceCase(value);
          else if (field === 'supplier' || field === 'customCatalog') finalValue = value.toUpperCase();
  ...
  const handleFinalSave = async () => {
    ...
    try {
        const finalFabrics: Fabric[] = extractedFabrics.map(data => ({
            ...
            colors: data.colors ? data.colors.map(toSentenceCase) : [],
            colorImages: data.colorImages || {},
            mainImage: data.mainImage || '',
            category: selectedCategory,
            customCatalog: (data.customCatalog || '').toUpperCase(),
            pdfUrl: data.pdfUrl
        }));

        if (finalFabrics.length === 1) await onSave(finalFabrics[0]);
        else if (finalFabrics.length > 1 && onBulkSave) await onBulkSave(finalFabrics);
  ...
  const handleAddFurniture = async () => {
      if (!newFurnitureName || !newFurnitureImage || !onSaveFurniture) return;
      setIsSaving(true);
      try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden relative flex flex-col h-auto min-h-[600px] max-h-[90vh]">
        ...
        <div className="flex flex-col border-b border-gray-100 bg-gray-50/50">
            ...
            <div className="flex px-8 space-x-8">
                <button 
                   ...
                   className={`pb-3 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'fabrics' ? 'border-black text-black' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                >
                    Subir Telas
                </button>
                <button 
                   onClick={() => setActiveTab('furniture')}
                   className={`pb-3 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'furniture' ? 'border-black text-black' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                >
                    Gestionar Muebles
                </button>
            </div>
            ...
            {activeTab === 'furniture' && (
                <div className="space-y-10">
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                        <h3 className="font-serif text-xl font-bold mb-6 text-slate-800">Agregar Mueble Nuevo</h3>
                        <div className="flex flex-col md:flex-row gap-6">
                            
                            <div 
                                onClick={() => furnitureInputRef.current?.click()}
                                className="w-32 h-32 flex-shrink-0 bg-white border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center cursor-pointer hover:border-black transition-colors relative overflow-hidden"
                            >
                                {newFurnitureImage ? (
                                    <img src={newFurnitureImage} className="w-full h-full object-cover" />
                                ) : (
                                    <div className="text-center">
                                        <svg className="w-6 h-6 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                                        <span className="text-[10px] font-bold text-gray-500 uppercase mt-1 block">Foto</span>
                                    </div>
                                )}
                    ...
                    <div>
                        <h3 className="font-serif text-xl font-bold mb-6 text-slate-800">Muebles Disponibles ({existingFurniture.length})</h3>
        ...
        {/* Footer Actions (Only for Fabrics tab when reviewing/uploading) */}
        {!isSaving && activeTab === 'fabrics' && step !== 'processing' && (
