...
const FabricCard: React.FC<FabricCardProps> = ({ fabric, onClick, mode, specificColorName }) => {
  ...
  return (
    ...
    >
      {/* SECTION SUPERIOR (Imagen) - Reduced to 60% to give text more room */}
      ...
      {/* SECTION INFERIOR (Información) - Increased height to 40% */}
        ...
        <div className="w-full flex flex-col justify-center h-full space-y-1">
          {mode === 'model' ? (
            ...
            <>
              ...
              {/* Added Supplier Name */}
              <p className="text-xs md:text-sm font-bold text-gray-400 uppercase tracking-widest mb-2 leading-none">
                {fabric.supplier}
              </p>
              
              <div className="w-12 h-[1px] bg-gray-200 mx-auto my-2"></div>
              
              <p className="text-xs md:text-sm text-gray-500 font-medium uppercase leading-snug px-1 tracking-wide line-clamp-2">
                {colorList.join(', ')}
              </p>
            </>
          ) : (
            ...
            <>
              <h3 className="font-serif text-xl md:text-2xl font-bold text-slate-900 leading-none mb-1 group-hover:text-black line-clamp-1 px-1">
               ...
               <div className="w-8 h-[1px] bg-gray-200 mx-auto my-2"></div>
